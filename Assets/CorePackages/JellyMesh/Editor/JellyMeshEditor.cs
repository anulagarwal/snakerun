			...
			if(jellyMesh.m_Style == JellyMesh.PhysicsStyle.Free)
			{
				if(jellyMesh.m_FreeModeBodyPositions != null)
				{
					...
					for(int i = 0; i < jellyMesh.m_FreeModeBodyPositions.Count; i++)
					{
						...
						var fmh_467_91_638102376495488110 = Quaternion.identity; Vector3 newPosition = transform.InverseTransformPoint(Handles.FreeMoveHandle(point, HandleUtility.GetHandleSize(point) * 0.05f, Vector3.zero, Handles.DotHandleCap));
